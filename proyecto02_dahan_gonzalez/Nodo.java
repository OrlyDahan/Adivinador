...
public class Nodo {
     ...
     */
    public void writeFile(Nodo root, PrintWriter w) throws FileNotFoundException{
        try{
        if (root == null){
            return;
        }
        boolean top = false; 
        if (w == null) {
            FileOutputStream outputStream = new FileOutputStream("BinaryTree.txt");
            w = new PrintWriter(outputStream); 
            top = true;  
        }
        writeFile(root.getLeft(), w);
        w.print(root);
        writeFile(root.getRight(), w);
